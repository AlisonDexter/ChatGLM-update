...
from ChatGLM3_main.tt import main as chat  #对显存要求高的
...
# 加载文档
def load_documents():
    #DOCUMENTS_FOLDER = r"D:/Age_copy/law_case"
    #DOCUMENTS_FOLDER = r"D:/Age_copy/law_doc"
    DOCUMENTS_FOLDER = r"D:/Age_copy/test"
...
def init_search_indices(documents, doc_vectors):
    ...
    if doc_vectors and len(doc_vectors) > 0:
        doc_vectors_np = np.array(doc_vectors, dtype=np.float32)  # 指定 float32 类型以提高性能
        dimension = doc_vectors_np.shape[1]
        
        # 使用更高效的索引类型
        faiss_index = faiss.IndexIVFFlat(faiss.IndexFlatL2(dimension), dimension, min(100, len(doc_vectors)))
        faiss_index.train(doc_vectors_np)
        faiss_index.add(doc_vectors_np)
        # 设置搜索时检查的单元格数量 - 增加精度但会降低速度
        faiss_index.nprobe = 10
        
        print(f"FAISS 索引已创建，包含 {len(doc_vectors)} 个向量")
        #faiss.write_index(faiss_index, "faiss_law_case.index")
        #faiss.write_index(faiss_index, "faiss_law_doc.index")
        faiss.write_index(faiss_index, "faiss_test.index")
        print("FAISS 索引已保存")
    ...
    if documents and len(documents) > 0:
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            tokenized_docs = list(executor.map(lambda doc: jieba.lcut(doc), documents.values()))
        
        if tokenized_docs:
            bm25 = BM25Okapi(tokenized_docs)
            print("BM25 索引创建成功")
    
    return faiss_index, bm25, tokenized_docs
...
faiss_index, bm25, tokenized_docs = init_search_indices(documents, doc_vectors)
...
def search():
    ...
    try:
        if not request.is_json:
            return jsonify({"error": "请求格式错误，必须使用 JSON"}), 400
        ...
        # 移除停用词
        query_tokens = [token for token in jieba.lcut(query) if token not in stopwords]
        query_for_bm25 = " ".join(query_tokens)
        ...
        if bm25:
            def bm25_search():
                ...
                # 只返回得分较高的结果
                threshold = max(scores) * 0.3 if scores else 0
                return [(doc_keys[i], scores[i]) for i in range(len(scores)) if scores[i] > threshold]
            futures.append(executor.submit(bm25_search))
        ...
        if faiss_index and len(doc_keys) > 0:
            def faiss_search():
                ...
                if query in query_vector_cache:
                    query_vector = query_vector_cache[query]
                else:
                    query_vector = model.encode([query])
                    # 缓存结果 (限制缓存大小)
                    if len(query_vector_cache) < 1000:
                        query_vector_cache[query] = query_vector
                
                # 搜索最相关的 k 个结果
                k = min(10, len(doc_keys))
                D, I = faiss_index.search(np.array(query_vector, dtype=np.float32), k=k)
                return [(doc_keys[idx], float(1 / (1 + D[0][i]))) for i, idx in enumerate(I[0]) if idx >= 0]
            futures.append(executor.submit(faiss_search))
        ...
        for doc_name, score in results:
            if doc_name in scored_results:
                scored_results[doc_name] = max(scored_results[doc_name], score)
            else:
                scored_results[doc_name] = score
        
        # 按得分排序
        sorted_results = sorted(scored_results.items(), key=lambda x: x[1], reverse=True)
        top_results = [doc_name for doc_name, _ in sorted_results[:10]]
        
        duration = time.time() - start_time
        print(f"搜索完成，用时: {duration:.3f}秒，结果: {len(top_results)} 个文档")
        return jsonify({"results": top_results, "time_taken": duration})
